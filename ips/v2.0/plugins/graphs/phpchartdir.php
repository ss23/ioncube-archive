...
if (!extension_loaded("ChartDirector PHP API"))
{
	...
	$track_error_save = ini_set("track_errors", 1);

	if (!$cdDebug)
	{
		ob_start();
		$debugQuery = $_SERVER["QUERY_STRING"];
		if ($debugQuery)
			$debugQuery .= "&cddebug=1";
		else
			$debugQuery = "cddebug=1";
?>
...
<ul><li>
Some unexpected error has occured. Please <a href="<?php echo $_SERVER['SCRIPT_NAME'].'?'.$debugQuery ?>">click here</a>
to try again in debug mode to get the error message. Then <a href="http://www.advsofteng.com/doc/cdphpdoc/phpdiag.htm">click here
</a> for online trouble-shooting instructions. 
</ul>
</div>
<?php
		$success = @dl($ext);
	...
	{
		$errmsg = $php_errormsg;
		$errlog = '<li>Error loading "'.$ext.'" from the PHP extension directory '.listExtDir().':<br><b>PHP error:</b> '.$errmsg;
		...
		if ($tryPath)
		{
			...
			{
				$errmsg = $php_errormsg;
				$errpath = ini_get("extension_dir");
				global $cdRelOp;
				$errlog = $errlog.'<br><br><li>Also try to load "'.$ext.'" from '.listRelExtDir($scriptPath).' but unsuccessful:<br><b>PHP error:</b> '.$errmsg;
			}
		}
	}
	...
	if ($success)
	{
		...
		if ($dllVersion != 0x0400)
		{
			$errlog = '<li>Version mismatch : "phpchartdir.php" is of version 4.0, but "'.
				(isOnWindows() ? "chartdir.dll" : "libchartdir.so").'" is of version '.
				(($dllVersion >> 8) & 0xff).'.'.($dllVersion & 0xff).'.';
			$success = 0;
		}
	}
	
	ini_set("track_errors", $track_error_save);
...
<div style="font-family:verdana; font-size:10pt;">
<ul>
<?php echo str_replace("<br><br><br>", "<br><br>", str_replace("\n", "<br>", cdFilterMsg($errlog))) ?>
</ul>
An attempt has been made to dynamically load ChartDirector, but it was not successful. Some common 
causes are that the security settings of PHP may have disabled loading dynamic extensions, or the 
ChartDirector libraries are not in the directory PHP expects. Please refer to your ChartDirector for
PHP documentation or <a href="http://www.advsofteng.com/doc/cdphpdoc/phpdiag.htm">click here</a> for
how to resolve the problem. The following information may be useful.
<ul>
...
function silverColor($angle = 90) {
	return metalColor(0xcccccc, $angle);
}
