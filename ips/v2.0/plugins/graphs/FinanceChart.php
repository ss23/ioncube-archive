...
class FinanceChart extends MultiChart
{
    ...
    function computeTriMovingAvg($data, $period) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($p);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($p);
    ...
    function computeWeightedMovingAvg($data, $period) {
        ...
        for($i = 2; $i < $period + 1; ++$i) {
            $tmpArrayMath1 = new ArrayMath($data);
            $tmpArrayMath1 = $tmpArrayMath1->movAvg($i);
            $tmpArrayMath1 = $tmpArrayMath1->mul($i);
    ...
    function setData($timeStamps, $highData, $lowData, $openData, $closeData, $volData, $extraPoints
        ...
        $tmpArrayMath1 = new ArrayMath($volData);
        $tmpArrayMath1 = $tmpArrayMath1->div(pow(1000, $unitIndex));
    ...
    function setXLabels(&$a) {
        ...
        if ($this->m_extraPoints < count($this->m_timeStamps)) {
            ...
            if ($secondsBetweenTicks > 90 * 86400) {
                #yearly ticks
                $a->setMultiFormat2(StartOfYearFilter(), $this->m_yearFormat, $tickStep);
            } else if ($secondsBetweenTicks > 7 * 86400) {
                #monthly ticks
            ...
            } else if ($secondsBetweenTicks > 6 * 3600) {
                #daily ticks
    ...
    function addOHLCLabel($upColor, $downColor, $candleStickMode) {
        ...
        if ($i >= 0) {
            ...
            if ($candleStickMode != true) {
                ...
                $tmpArrayMath1 = $tmpArrayMath1->delta();
            ...
            $getLegendObj = $this->m_mainChart->getLegend();
            $getLegendObj->addKey($label, $udcolor);
        }
    }
    ...
    function addWeightedClose($color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->div(4);
    ...
    function addTypicalPrice($color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->div(3);
        return $this->addLineIndicator2($this->m_mainChart, $tmpArrayMath1->result(), $color,
            "Typical Price");
    }
    ...
    function addMedianPrice($color) {
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->div(2);
        return $this->addLineIndicator2($this->m_mainChart, $tmpArrayMath1->result(), $color,
            "Median Price");
    }
    ...
    function addSimpleMovingAvg($period, $color) {
        $label = "SMA ($period)";
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period);
    ...
    function addExpMovingAvg($period, $color) {
        $label = "EMA ($period)";
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->expAvg(2.0 / ($period + 1));
        return $this->addLineIndicator2($this->m_mainChart, $tmpArrayMath1->result(), $color, $label
            );
    }
    ...
    function addBollingerBand($period, $bandWidth, $lineColor, $fillColor) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->movStdDev($period);
        $tmpArrayMath1 = $tmpArrayMath1->mul($bandWidth);
        ...
        $tmpArrayMath1 = $tmpArrayMath1->add($stdDev);
        $tmpArrayMath2 = new ArrayMath($movAvg);
        $tmpArrayMath2 = $tmpArrayMath2->sub($stdDev);
    ...
    function addDonchianChannel($period, $lineColor, $fillColor) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->movMax($period);
        $tmpArrayMath2 = new ArrayMath($this->m_lowData);
        $tmpArrayMath2 = $tmpArrayMath2->movMin($period);
        return $this->addBand($tmpArrayMath1->result(), $tmpArrayMath2->result(), $lineColor,
            $fillColor, $label);
    }
    ...
    function addEnvelop($period, $range, $lineColor, $fillColor) {
        ...
        $tmpArrayMath1 = new ArrayMath($movAvg);
        $tmpArrayMath1 = $tmpArrayMath1->mul(1 + $range);
        $tmpArrayMath2 = new ArrayMath($movAvg);
        $tmpArrayMath2 = $tmpArrayMath2->mul(1 - $range);
        return $this->addBand($tmpArrayMath1->result(), $tmpArrayMath2->result(), $lineColor,
            $fillColor, $label);
    }
    ...
    function addVolBars2(&$c, $height, $upColor, $downColor, $flatColor) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->selectGTZ($closeChange);
        $upLayer = $this->addBarIndicator2($c, $tmpArrayMath1->result(), $upColor, "");
        $tmpArrayMath1 = new ArrayMath($this->m_volData);
        $tmpArrayMath1 = $tmpArrayMath1->selectLTZ($closeChange);
        $dnLayer = $this->addBarIndicator2($c, $tmpArrayMath1->result(), $downColor, "");
        $tmpArrayMath1 = new ArrayMath($this->m_volData);
        $tmpArrayMath1 = $tmpArrayMath1->selectEQZ($closeChange);
        ...
        if ($c == $this->m_mainChart) {
            ...
            $getDrawAreaObj = $c->getDrawArea();
            $topMargin = $getDrawAreaObj->getHeight() - $this->m_topMargin - $this->m_bottomMargin -
                $height + $this->m_yAxisMargin;
            if ($topMargin < 0) {
                $topMargin = 0;
            }
            $yAxis2Obj = $c->yAxis2();
            $yAxis2Obj->setTopMargin($topMargin);
        ...
        $getDataSetObj = $selectedLayer->getDataSet(0);
        $getDataSetObj->setDataName($label);
    ...
    function addAccDist($height, $color) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_lowData);
        $range = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->mul(2);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->mul($this->m_volData);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($range, 0);
        $tmpArrayMath1 = $tmpArrayMath1->acc();
    ...
    function addAroonOsc($height, $period, $color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->computeAroonDn($period));
    ...
    function computeTrueRange() {
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->shift();
    ...
    function addADX($height, $period, $posColor, $negColor, $color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->delta();
        $tmpArrayMath1 = $tmpArrayMath1->mul(-1);
        $neg = $tmpArrayMath1->selectGTZ();
        $tmpArrayMath1 = new ArrayMath($pos->result());
        $tmpArrayMath1 = $tmpArrayMath1->sub($neg->result());
        ...
        $tmpArrayMath1 = new ArrayMath($pos->result());
        $tmpArrayMath1 = $tmpArrayMath1->add($neg->result());
        $totalDM = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($pos->result());
        $tmpArrayMath1 = $tmpArrayMath1->sub($neg->result());
        $tmpArrayMath1 = $tmpArrayMath1->abs();
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($totalDM, 0);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100);
    ...
    function addBollingerWidth($height, $period, $width, $color) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->movStdDev(20);
        $tmpArrayMath1 = $tmpArrayMath1->mul($width * 2);
    ...
    function addCCI($height, $period, $color, $range, $upColor, $downColor) {
        ...
        $label = "CCI ($period)";
        $tmpArrayMath1 = new ArrayMath($tp);
        $tmpArrayMath1 = $tmpArrayMath1->sub($smvtp);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($movMeanDev, 0);
        $tmpArrayMath1 = $tmpArrayMath1->div(0.015);
    ...
    function addChaikinMoneyFlow($height, $period, $color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($range, 0);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($volAvg, 0);
    ...
    function addChaikinOscillator($height, $color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->expAvg(2.0 / (11));
        $expAvg10 = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($accdist);
        $tmpArrayMath1 = $tmpArrayMath1->expAvg(2.0 / (4));
        $tmpArrayMath1 = $tmpArrayMath1->sub($expAvg10);
        return $this->addLineIndicator($height, $tmpArrayMath1->result(), $color,
            "Chaikin Oscillator");
    }
    ...
    function addChaikinVolatility($height, $period1, $period2, $color) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->expAvg(2.0 / ($period1 + 1));
        $tmpArrayMath1 = $tmpArrayMath1->rate($period2);
        $tmpArrayMath1 = $tmpArrayMath1->sub(1);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100);
        return $this->addLineIndicator($height, $tmpArrayMath1->result(), $color, $label);
    }
    ...
    function addDPO($height, $period, $color) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period);
        $tmpArrayMath1 = $tmpArrayMath1->shift($period / 2 + 1);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->mul(-1);
        return $this->addLineIndicator($height, $tmpArrayMath1->result(), $color, $label);
    }
    ...
    function addDonchianWidth($height, $period, $color) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->movMax($period);
        $tmpArrayMath1 = $tmpArrayMath1->sub($tmpArrayMath2->result());
        return $this->addLineIndicator($height, $tmpArrayMath1->result(), $color, $label);
    }
    ...
    function addEaseOfMovement($height, $period, $color1, $color2) {
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($this->m_volData, 0);
        $boxRatioInverted = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->add($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->div(2);
        $tmpArrayMath1 = $tmpArrayMath1->delta();
        $tmpArrayMath1 = $tmpArrayMath1->mul($boxRatioInverted);
    ...
    function addFastStochastic($height, $period1, $period2, $color1, $color2) {
        $tmpArrayMath1 = new ArrayMath($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->movMin($period1);
        $movLow = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->movMax($period1);
        $tmpArrayMath1 = $tmpArrayMath1->sub($movLow);
        $movRange = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($movLow);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($movRange, 0.5);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100);
        ...
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period2);
    ...
    function addMACD($height, $period1, $period2, $period3, $color, $signalColor, $divColor) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->expAvg(2.0 / ($period2 + 1));
        $tmpArrayMath1 = $tmpArrayMath1->sub($expAvg1);
        ...
        $tmpArrayMath1 = $tmpArrayMath1->expAvg(2.0 / ($period3 + 1));
        ...
        #Divergence
        $tmpArrayMath1 = new ArrayMath($macd);
        $tmpArrayMath1 = $tmpArrayMath1->sub($macdSignal);
    ...
    function addMassIndex($height, $color, $upColor, $downColor) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_highData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($this->m_lowData);
        $tmpArrayMath1 = $tmpArrayMath1->expAvg($f);
        ...
        $tmpArrayMath1 = new ArrayMath($exp9);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($exp99, 1);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg(25);
        $tmpArrayMath1 = $tmpArrayMath1->mul(25);
    ...
    function addMFI($height, $period, $color, $range, $upColor, $downColor) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->selectGTZ($selector);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period);
        $posMoneyFlow = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($moneyFlow);
        $tmpArrayMath1 = $tmpArrayMath1->selectLTZ($selector);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period);
        $tmpArrayMath1 = $tmpArrayMath1->add($posMoneyFlow);
        $posNegMoneyFlow = $tmpArrayMath1->result();

        $c = $this->addIndicator($height);
        $label = "Money Flow Index ($period)";
        $tmpArrayMath1 = new ArrayMath($posMoneyFlow);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($posNegMoneyFlow, 0.5);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100);
        $layer = $this->addLineIndicator2($c, $tmpArrayMath1->result(), $color, $label);
    ...
    function addMomentum($height, $period, $color) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->delta($period);
        return $this->addLineIndicator($height, $tmpArrayMath1->result(), $color, $label);
    }
    ...
    function addOBV($height, $color) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->sub($downVolume);
        $tmpArrayMath1 = $tmpArrayMath1->acc();
    ...
    function addPerformance($height, $color) {
        ...
        if ($firstCloseIndex < count($this->m_closeData)) {
            $tmpArrayMath1 = new ArrayMath($this->m_closeData);
            $tmpArrayMath1 = $tmpArrayMath1->div($this->m_closeData[$firstCloseIndex]);
            $tmpArrayMath1 = $tmpArrayMath1->sub(1);
            $tmpArrayMath1 = $tmpArrayMath1->mul(100);
    ...
    function addPPO($height, $period1, $period2, $period3, $color, $signalColor, $divColor) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->sub($expAvg1);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($expAvg2, 0);
    ...
    function addPVT($height, $color) {
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->rate();
        $tmpArrayMath1 = $tmpArrayMath1->sub(1);
        $tmpArrayMath1 = $tmpArrayMath1->mul($this->m_volData);
        $tmpArrayMath1 = $tmpArrayMath1->acc();
    ...
    function addROC($height, $period, $color) {
        $label = "ROC ($period)";
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->rate($period);
        $tmpArrayMath1 = $tmpArrayMath1->sub(1);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100);
        return $this->addLineIndicator($height, $tmpArrayMath1->result(), $color, $label);
    }
    ...
    function computeRSI($period) {
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->delta();
        $tmpArrayMath1 = $tmpArrayMath1->selectGTZ();
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($absChange, 0.5);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100);
        return $tmpArrayMath1->result();
    }
    ...
    function addStochRSI($height, $period, $color, $range, $upColor, $downColor) {
        ...
        $tmpArrayMath1 = $tmpArrayMath1->movMin($period);
        $movLow = $tmpArrayMath1->result();
        $tmpArrayMath1 = new ArrayMath($rsi);
        $tmpArrayMath1 = $tmpArrayMath1->movMax($period);
        $tmpArrayMath1 = $tmpArrayMath1->sub($movLow);
    ...
    function computeTrueLow() {
        ...
        for($i = 0; $i < count($this->m_lowData); ++$i) {
            if (($this->m_volData[$i] != NoValue) & ($previousClose[$i] != NoValue)) {
                if ($this->m_volData[$i] < $previousClose[$i]) {
    ...
    function addUltimateOscillator($height, $period1, $period2, $period3, $color, $range, $upColor,
        ...
        $tmpArrayMath1 = new ArrayMath($this->m_closeData);
        $tmpArrayMath1 = $tmpArrayMath1->sub($trueLow);
        $buyingPressure = $tmpArrayMath1->result();
        $trueRange = $this->computeTrueRange();

        $tmpArrayMath2 = new ArrayMath($trueRange);
        $tmpArrayMath2 = $tmpArrayMath2->movAvg($period1);
        $tmpArrayMath1 = new ArrayMath($buyingPressure);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period1);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($tmpArrayMath2->result(), 0.5);
        $tmpArrayMath1 = $tmpArrayMath1->mul(4);
        $rawUO1 = $tmpArrayMath1->result();
        $tmpArrayMath2 = new ArrayMath($trueRange);
        $tmpArrayMath2 = $tmpArrayMath2->movAvg($period2);
        $tmpArrayMath1 = new ArrayMath($buyingPressure);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period2);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($tmpArrayMath2->result(), 0.5);
        $tmpArrayMath1 = $tmpArrayMath1->mul(2);
        ...
        $tmpArrayMath2 = new ArrayMath($trueRange);
        $tmpArrayMath2 = $tmpArrayMath2->movAvg($period3);
        $tmpArrayMath1 = new ArrayMath($buyingPressure);
        $tmpArrayMath1 = $tmpArrayMath1->movAvg($period3);
        $tmpArrayMath1 = $tmpArrayMath1->financeDiv($tmpArrayMath2->result(), 0.5);
        $tmpArrayMath1 = $tmpArrayMath1->mul(1);
        ...
        $tmpArrayMath1 = $tmpArrayMath1->add($rawUO2);
        $tmpArrayMath1 = $tmpArrayMath1->add($rawUO3);
        $tmpArrayMath1 = $tmpArrayMath1->mul(100.0 / 7);
        $layer = $this->addLineIndicator2($c, $tmpArrayMath1->result(), $color, $label);
        $this->addThreshold($c, $layer, 50 + $range, $upColor, 50 - $range, $downColor);
