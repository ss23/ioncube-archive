...
if (!extension_loaded("ChartDirector PHP API"))
{
	...
	//1) If extension_dir absolute, then tell them path 
	//2) If relative, then tell them to make extension_dir absolute 
	//   and put the dll in that directory, and 

	$scriptPath = dirname(__FILE__);
	$dlldir = $scriptPath."/../../plugins/graphs";
	$tryPath = getRelExtPath($dlldir);

	$dllpath = "$dlldir/$ext";

	if ($tryPath)
	{
		$full_path = $tryPath."/$ext";
		@dl($full_path);
		$success = extension_loaded("ChartDirector PHP API");
		if (!$success)
		{
			$extdir = ini_get("extension_dir");
			$absolute = false;
			if ($extdir[0] == "/" || $extdir[0] == "\\")
				$absolute = true;
			if ($extdir[1] == ":" && ctype_alpha($extdir[0]))
				$absolute = true;

			echo("Unable to load the graphs module.<br><br>"); 
			if ($absolute)
			{  
				echo("Please copy the file <b>$dllpath</b> into the directory <b>$extdir</b>, add the following line to your php.ini:<br><br>extension=$ext<br><br>then restart your web server software.");
			}
			else
			{
				echo("Please set extension_dir to an absolute path in your php.ini file, 
				copy the file <b>$dllpath</b> into the extension directory, add the following line to your php.ini:<br><br>extension=$ext<br><br>then restart your web server software.");
			}
			exit(1);
		}
	}
